...
export class DirectorController {
    ...
    static async actualizarPerfil(c) {
        try {
            ...
            const director = await prisma.usuarios.update({
                where: {
                    id: user.id
                },
                data: {
        ...
        try {
            ...
            const estadisticas = await Promise.race([
                (async () => {
                    ...
                    const [totalEstudiantes, totalProfesores, totalApoderados, totalGrados, totalSecciones, estudiantesActivos, profesoresActivos] = await Promise.all([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout: Consulta de estadísticas tardó demasiado')), 10000))
            ]);
    ...
    static async crearAsignacion(c) {
        try {
    ...
    static async actualizarAsignacion(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    // Eliminar asignación
    static async eliminarAsignacion(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    // Obtener secciones
    static async obtenerSecciones(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async obtenerAsistenciaPorSalon(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async obtenerEstadisticas(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
