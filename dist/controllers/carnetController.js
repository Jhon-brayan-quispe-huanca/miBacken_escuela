    ...
    static async generarCarnetEstudiante(c) {
        ...
        try {
            ...
            const estudianteExiste = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout: Validación tardó demasiado')), 5000))
            ]);
            if (!estudianteExiste) {
            ...
            const pdfBuffer = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout: Generación de PDF tardó demasiado')), 30000))
            ]);
