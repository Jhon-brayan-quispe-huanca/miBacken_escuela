...
export class PorteroController {
    ...
    static async marcarAsistenciaJustificadaAutomaticamente(estudianteId, permiso) {
    ...
    static async actualizarPerfil(c) {
        try {
            ...
            const { nombres, apellidos, email, telefono, direccion, fecha_nacimiento, genero } = await c.req.json();
    ...
    static async procesarEscaneoQR(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
            }
            const datosQR = await c.req.json();
    ...
    static async obtenerAsistenciasHoy(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
    ...
    static async obtenerHistorialEstudiante(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
            ...
            const historial = await AsistenciaService.obtenerHistorialAsistencia(estudianteId, fechaInicio, fechaFin);
    ...
    static async obtenerHistorialCompletoEstudiante(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
            ...
            const historial = await AsistenciaService.obtenerHistorialCompletoAsistencia(estudianteId, fechaInicio, fechaFin);
            return c.json({
                success: true,
                historial
            });
    ...
    static async buscarEstudiantePorCodigo(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
    ...
    static async buscarEstudiantesPorNombre(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) {
                return c.json({ message: 'Acceso denegado. Solo porteros pueden acceder.' }, 403);
            ...
            const estudiantesFormateados = await Promise.all(estudiantes.map(async (estudiante) => {
                ...
                if (tienePermisoActivo) {
                    ...
                    if (!asistenciaHoy) {
                        asistenciaHoy = await this.marcarAsistenciaJustificadaAutomaticamente(estudiante.id, estudiante.solicitudes_permisos[0]);
                    }
                }
                return {
                    id: estudiante.id,
    ...
    static async registrarAsistenciaManual(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) { // rol_id 3 es para porteros
            ...
            if (tienePermisoActivo) {
                ...
                if (tipoAsistencia === 'entrada' || tipoAsistencia === 'tardanza') {
                    ...
                    const asistenciaJustificada = await this.marcarAsistenciaJustificadaAutomaticamente(parseInt(estudianteId), estudiante.solicitudes_permisos[0]);
                    return c.json({
                        success: true,
            ...
            try {
                ...
                if (estudianteConApoderado?.apoderados?.usuario_id) {
                    ...
                    await NotificacionService.crearNotificacionAsistencia(parseInt(estudianteId), estudianteConApoderado.apoderados.usuario_id, {
                        hora: horaFormateada,
     ...
     */
    static async marcarAusenciasManual(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 3) { // rol_id 3 es para porteros
                return c.json({
...
//# sourceMappingURL=porteroController.js.map