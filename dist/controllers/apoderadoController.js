...
apoderadoController.get('/asistencias-generales', async (c) => {
    try {
        ...
        const estadisticas = {
            ...
            presentes: asistenciasFormateadas.filter((a) => a.estado_original === 'Presente').length,
            tardanzas: asistenciasFormateadas.filter((a) => a.estado_original === 'Tarde').length,
            ausencias: asistenciasFormateadas.filter((a) => a.estado_original === 'Ausente').length,
            justificadas: asistenciasFormateadas.filter((a) => a.estado_original === 'Justificado').length,
            conSalida: asistenciasFormateadas.filter((a) => a.hora_salida !== null).length
        };
        return c.json({
            success: true,
            data: {
...
apoderadoController.get('/carnets', async (c) => {
    try {
        ...
        // Formatear respuesta
        const carnetsFormateados = carnets.map((carnet) => ({
            id: carnet.id,
...
//# sourceMappingURL=apoderadoController.js.map