...
notificacionController.post('/crear', async (c) => {
    try {
        const body = await c.req.json();
        const { usuario_id, titulo, mensaje, tipo, estudiante_id, asistencia_id, prioridad, categoria, datos_adicionales, accion_requerida, } = body;
...
notificacionController.post('/asistencia', async (c) => {
    try {
        const body = await c.req.json();
        const { estudiante_id, apoderado_id, hora, estado, observaciones, } = body;
        // Validaciones básicas
        ...
        const resultado = await NotificacionService.crearNotificacionAsistencia(estudiante_id, apoderado_id, {
            hora,
            estado,
            observaciones,
        });
...
notificacionController.post('/permiso', async (c) => {
    try {
        ...
        const resultado = await NotificacionService.crearNotificacionPermiso(solicitud_id, tipo);
        if (!resultado.success) {
            return c.json(resultado, 500);
        }
        return c.json(resultado, 201);
