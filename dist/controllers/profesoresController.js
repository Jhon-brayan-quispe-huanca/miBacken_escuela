...
export class ProfesoresController {
    ...
    static async obtenerProfesores(c) {
        try {
    ...
    static async obtenerProfesorPorId(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async crearProfesor(c) {
        try {
            ...
            const { dni, nombres, apellidos, email, telefono, direccion, fecha_nacimiento, genero, especialidad, fecha_ingreso, codigo_profesor, tipo_profesor } = body;
    ...
    // Actualizar profesor
    static async actualizarProfesor(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
            ...
            const id = parseInt(c.req.param('id'));
            const body = await c.req.json();
            const { nombres, apellidos, email, telefono, direccion, fecha_nacimiento, genero, especialidad, fecha_ingreso, codigo_profesor, tipo_profesor } = body;
    ...
    static async cambiarEstadoProfesor(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async eliminarProfesor(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async asignarProfesorGradoSeccion(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
            ...
            const body = await c.req.json();
            const { grado_id, seccion_id, es_tutor, anio_escolar } = body;
            // Validaciones básicas
    ...
    static async obtenerMaterias(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async obtenerAsignacionesProfesor(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
    ...
    static async eliminarAsignacionProfesor(c) {
        try {
            const user = c.get('user');
            if (!user || user.rol_id !== 1) {
                return c.json({ message: 'Acceso denegado. Solo directores pueden acceder.' }, 403);
...
//# sourceMappingURL=profesoresController.js.map