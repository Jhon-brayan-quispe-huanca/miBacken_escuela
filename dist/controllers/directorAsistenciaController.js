...
export class DirectorAsistenciaController {
    ...
    static async obtenerAsistenciasGenerales(c) {
        try {
                ...
                for (const estudiante of todosEstudiantes) {
                    if (!estudiantesConAsistencia.has(estudiante.id)) {
                        ...
                        if (debeSerAusente) {
                            ...
                            asistenciasGenerales.push(ausenteAutomatico);
                        }
                    }
                }
            }
            ...
            if (estado && estado !== 'Todos') {
                if (estado === 'Salida') {
                    ...
                    asistenciasFormateadas = asistenciasFormateadas.filter(asistencia => asistencia.hora_salida !== null);
                }
                else {
                    // Para otros estados, usar el estado normal
                    asistenciasFormateadas = asistenciasFormateadas.filter(asistencia => asistencia.estado === estado);
                }
            }
            // Aplicar filtro por turno si se especifica (filtro adicional en frontend)
            if (turno && turno !== 'Todos') {
                const gradosPorTurno = turno === 'Mañana' ? [1, 3, 6] : [2, 4, 5];
                asistenciasFormateadas = asistenciasFormateadas.filter(asistencia => gradosPorTurno.includes(asistencia.estudiante.gradoId));
            }
    ...
    static async obtenerAsistenciasPorFecha(c) {
        try {
            const fecha = c.req.query('fecha');
    ...
    static async justificarAsistencia(c) {
        try {
    ...
    static async obtenerEstadisticasAsistencias(c) {
        try {
            const fecha = c.req.query('fecha');
            if (!fecha) {
                return c.json({
    ...
    static async obtenerProfesoresConAsistencia(c) {
        try {
            const fecha = c.req.query('fecha');
            const gradoId = c.req.query('grado_id');
            const seccionId = c.req.query('seccion_id');
    ...
    static async obtenerAsistenciasPorSalon(c) {
        ...
        try {
            ...
            const presentes = asistenciasFormateadas.filter((a) => a.estado_asistencia === 'Presente').length;
            const ausentes = asistenciasFormateadas.filter((a) => a.estado_asistencia === 'Ausente').length;
            const tardanzas = asistenciasFormateadas.filter((a) => a.estado_asistencia === 'Tardanza').length;
            const justificados = asistenciasFormateadas.filter((a) => a.estado_asistencia === 'Justificado').length;
            const sinRegistrar = asistenciasFormateadas.filter((a) => a.estado_asistencia === 'Sin registrar').length;
            const estadisticas = {
                total,
                presentes,
                ausentes,
    ...
    static async exportarAsistenciaExcel(c) {
        try {
            const gradoId = c.req.query('grado_id');
            const seccionId = c.req.query('seccion_id');
            const fecha = c.req.query('fecha');
            ...
            if (tipo === 'mensual') {
                ...
                const excelBuffer = await DirectorReporte.generarReporteConExcelJS(parseInt(gradoId), parseInt(seccionId), parseInt(mes), parseInt(año), profesorIdParam);
            ...
            if (tipo === 'mensual') {
                ...
                const profesorIdParam = profesorId ? parseInt(profesorId) : undefined;
                const datosMensualesAOA = await DirectorReporte.generarReporteMensual(parseInt(gradoId), parseInt(seccionId), parseInt(mes), parseInt(año), profesorIdParam);
...
//# sourceMappingURL=directorAsistenciaController.js.map