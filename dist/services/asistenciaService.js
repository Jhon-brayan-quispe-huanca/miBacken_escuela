...
import { getFechaActualPeru, getFechaHoraActualPeru, formatearFechaPeru, formatearHoraPeru, getInicioDiaPeru, getFinDiaPeru, getHoraActualPeruParaBD } from '../utils/.js';
const prisma = new PrismaClient();
export class AsistenciaService {
    /**
     * Procesa el escaneo de QR y registra la asistencia
     */
    static async procesarEscaneoQR(datosQR, porteroId) {
        try {
            ...
            const { esHorarioEntrada, esHorarioSalida, estado } = this.determinarEstadoAsistencia(horaActualPeru, datosQR.turno);
     ...
     */
    static determinarEstadoAsistencia(hora, turno) {
     ...
     */
    static async notificarApoderado(estudiante, tipo, hora) {
        try {
    ...
    static async obtenerAsistenciasHoy() {
        try {
            ...
            asistencias.forEach(asistencia => {
                ...
                let estado_asistencia = 'Presente';
     ...
     */
    static async obtenerHistorialAsistencia(estudianteId, fechaInicio, fechaFin) {
        try {
            ...
            for (const asistencia of asistencias) {
                ...
                if (asistencia.hora_entrada && asistencia.hora_salida) {
                    ...
                    const estadoAsistencia = asistencia.estado;
                    historialFormateado.push({
                        id: asistencia.id,
                        fecha: asistencia.fecha.toLocaleDateString('es-PE', {
                            timeZone: 'America/Lima',
     ...
     */
    static async obtenerHistorialCompletoAsistencia(estudianteId, fechaInicio, fechaFin) {
        try {
     ...
     */
    static async obtenerAsistenciasPorFecha(fecha) {
        try {
     ...
     */
    static async marcarAusenciasAutomaticas() {
        try {
            ...
            const estudiantesSinAsistencia = todosLosEstudiantes.filter(estudiante => !idsConAsistencia.has(estudiante.id));
