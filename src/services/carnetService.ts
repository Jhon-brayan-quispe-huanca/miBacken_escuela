...
export class CarnetService {
   ...
   */
  async obtenerDatosEstudiante(estudianteId: number): Promise<EstudianteCarnetData | null> {
    try {
   ...
   */
  async generarQR(datosEstudiante: EstudianteCarnetData): Promise<string> {
    try {
   ...
   */
  async generarPDFCarnet(estudianteId: number): Promise<Buffer> {
    let browser;
    try {
   ...
   */
  async validarEstudiante(estudianteId: number): Promise<boolean> {
    try {
      const estudiante = await prisma.estudiantes.findUnique({
        where: { id: estudianteId }
      });
   ...
   */
  async generarImagenCarnet(estudiante: any): Promise<Buffer> {
    let browser;
    try {
