...
export class CarnetMasivoController {
   ...
   */
  static async obtenerEstudiantesFiltrados(c: Context) {
    try {
      ...
      const estudiantes = await prisma.estudiantes.findMany({
        where: {
          grado_id: parseInt(gradoId as string),
          seccion_id: parseInt(seccionId as string)
        },
        include: {
          grados: {
            select: {
   ...
   */
  static async generarCarnetsMasivo(c: Context) {
    try {
   ...
   */
  static async obtenerEstadisticasFiltro(c: Context) {
    try {
      const { gradoId, seccionId } = c.req.query();
      ...
      if (gradoId && gradoId !== 'null') {
        whereClause.grado_id = parseInt(gradoId as string);
      }
      
      if (seccionId && seccionId !== 'null') {
        whereClause.seccion_id = parseInt(seccionId as string);
      }
