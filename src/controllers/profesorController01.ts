...
export class ProfesorController01 {
  ...
  static async marcarAsistenciaJustificadaAutomaticamente(estudianteId: number, profesorId: number, permiso: any) {
   ...
   */
  static async obtenerProfesorPorUsuarioId(c: Context) {
    try {
   ...
   */
  static async obtenerEstudiantesAsignacion(c: Context) {
    try {
      const user = c.get('user');
        ...
        if (tienePermisoActivo) {
          ...
          if (!asistenciaHoy) {
            ...
            if (e.solicitudes_permisos && e.solicitudes_permisos.length > 0) {
              asistenciaHoy = await ProfesorController01.marcarAsistenciaJustificadaAutomaticamente(
                e.id, 
                profesor.id,
                e.solicitudes_permisos[0]
              );
            }
   ...
   */
  static async registrarAsistencia(c: Context) {
    try {
      const user = c.get('user');
      ...
      for (const est of estudiantes) {
        ...
        if (tienePermisoActivo && estudianteConPermiso) {
          ...
          await ProfesorController01.marcarAsistenciaJustificadaAutomaticamente(
            est.estudianteId,
            profesor.id,
            estudianteConPermiso.solicitudes_permisos[0]
          );
   ...
   */
  static async obtenerAsignacionesParaFiltro(c: Context) {
    try {
      const user = c.get('user');
   ...
   */
  static async obtenerReportes(c: Context) {
    try {
      ...
      if (asignacionId) {
        ...
        const asignacion = await prisma.profesor_grado_seccion.findFirst({
          where: {
            id: parseInt(asignacionId as string),
            profesor_id: profesor.id
          }
        });
        ...
        const whereClauseAsistencias: any = {
          profesor_id: profesor.id,
          estudiante_id: {
   ...
   */
  static async obtenerEstadisticasAsistencia(c: Context) {
    try {
      const user = c.get('user');
   ...
   */
  static async actualizarAsistencia(c: Context) {
    try {
      const user = c.get('user');
  ...
  static async obtenerEstadisticasDashboard(c: Context) {
    try {
      ...
      const estadisticasPorAsignacion = await Promise.all(
        asignaciones.map(async (asignacion) => {
          const estudiantesAsignacion = estudiantes.filter(
            e => e.grado_id === asignacion.grado_id && e.seccion_id === asignacion.seccion_id
          );
