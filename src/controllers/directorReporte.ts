...
export class DirectorReporte {
  ...
  static async generarReporteMensual(
    ...
    try {
      ...
      for (let i = 0; i < estudiantes.length; i++) {
        ...
        asistenciasEstudiante.forEach((asistencia: any) => {
        ...
        const filaAsistencias: string[] = [];

        // Procesar cada semana
        semanas.forEach(semana => {
          semana.dias.forEach((dia: Date) => {
        ...
        const totalPresentes = asistenciasEstudiante.filter((a: any) => a.estado === 'Presente').length;
        const totalJustificados = asistenciasEstudiante.filter((a: any) => a.estado === 'Justificado').length;
        const totalAusentes = asistenciasEstudiante.filter((a: any) => a.estado === 'Ausente').length;
        const totalTardanzas = asistenciasEstudiante.filter((a: any) => a.estado === 'Tardanza').length;
  ...
  private static _obtenerNombreMes(mes: number): string {
    const meses = [
  ...
  private static _mapearEstadoAbreviatura(estado: string): string {
  ...
  static aplicarEstilosExcel(workbook: any, worksheet: any): void {
    try {
      ...
      for (let col = 0; col <= 30; col++) {
        if (col === 0) colWidths.push({ wch: 4 }); // N°
        else if (col === 1) colWidths.push({ wch: 20 }); // ALUMNO
        else if (col >= 2 && col <= 26) colWidths.push({ wch: 6 }); // Días de la semana
        else if (col >= 27 && col <= 28) colWidths.push({ wch: 8 }); // TOTAL F y T
  ...
  static async incluirLogoEnExcel(workbook: any, worksheet: any): Promise<void> {
    try {
  ...
  static async generarReporteConExcelJS(
    gradoId: number, 
    seccionId: number, 
    mes: number, 
    año: number, 
  ...
  private static _aplicarEstilosMejorados(worksheet: any): void {
    try {
  ...
  private static _centrarTitulos(worksheet: any): void {
    try {
  ...
  private static _corregirTotales(worksheet: any): void {
    try {
      const range = XLSX.utils.decode_range(worksheet['!ref'] || 'A1:Z100');
        ...
        for (let col = range.s.c; col <= range.e.c; col++) {
          ...
          if (cell && cell.v === 0 && col >= 27) { // Columnas de totales
            ...
            for (let c = 2; c < 27; c++) { // Columnas de asistencias
              ...
              if (asistenciaCell && asistenciaCell.v === 'A') faltas++;
              if (asistenciaCell && asistenciaCell.v === 'T') tardanzas++;
            }
            ...
            if (col === 27) cell.v = faltas; // TOTAL F
            if (col === 28) cell.v = tardanzas; // TOTAL T
          }
        }
      }
